...
void* __default_alloc_template<threads, inst>::reallocate(void* __p , size_t __old_sz , size_t __new_sz )
{
    ...
    __result = ::allocate(__new_sz); //极有可能调用的时一级配置器的malloc_alloc::allocate , 这里可能写错了
	
    __copy_sz = __new_sz > __old_sz ? __old_sz : __new_sz;
	
    memcpy( __result, __p, __copy_sz );

	::deallocate(__p, __old_sz); //极有可能调用的时一级配置器的malloc_alloc::deallocate , 这里可能写错了
