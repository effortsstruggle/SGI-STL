...
// allocators.
...
#ifdef __STL_USE_STD_ALLOCATORS 	//_Vector_base 通过继承的方式，确定配置器
	...
	protected:
		allocator_type _M_data_allocator; //配置器

		_Tp* _M_start; //目前已使用空间的头

		_Tp* _M_finish; //目前已使用空间的尾

		_Tp* _M_end_of_storage; //目前可用空间的尾 （_M_end_of_storage - _M_finish） 备用空间 
	...
	protected:
		...
		typedef typename _Alloc_traits<_Tp, _Allocator>::_Alloc_type _Alloc_type; //simple_alloc<>;
...
#else /* __STL_USE_STD_ALLOCATORS */  // _Vector_base通过泛型的方式，确定空间配置器
	...
	protected:
		_Tp* _M_start;  // 表示目前已使用空间的头

		_Tp* _M_finish; // 表示目前已使用空间的尾
...
/**
* _Alloc : 空间配置器 本质上都是调用 一级或二级配置器， 一般默认使用的时二级配置器 ，看编译器是由定义了__USE_MALLOC 宏
*/
//SGI STL 的空间配置器 __STL_DEFAULT_ALLOCATOR   allocator<T>
...
public:
	...
	typedef value_type* iterator; // vector 的迭代器是普通指针
									//(迭代器由容器自己定义 , random_access_iterator_tag ）
...
public: // vector 操作函数的实现
  ...
  { 
	...
	// 构造拥有范围 [first, last) 内容的容器。 //type_traits 
	...
	/**
	* @breif insert 在 __position 前插入 __x 
	* @author wq
	* @Date	2023 / 07 / 22
	* @param __position 指定插入的位置
	* @param __x 插入的值
	* @return 返回插入位置的指针
	*/
	iterator insert(iterator __position, const _Tp& __x) 
	{
		...
		if (_M_finish != _M_end_of_storage && __position == end()) //插入在末尾
		{
	...
	// 清除某个位置上的元素(就是将__position+1 ~ _M_finish 往前移动一位后，在释放_M_finish位置的空间)
	...
	{
		iterator __i = copy(__last, _M_finish, __first); //当__last == _M_finish时，这句话是没用的
	...
	{
		if ( __new_size < size() ) //新的容器个数 < 当前容器个数时，删除 
		  erase( begin() + __new_size, end() );
		else //新的容器个数        >  当前容器个数时，插入
	...
	if (__n != 0) 
	{
		...
		else
		{
			...
			__STL_TRY 
			{
				...
				__new_finish = uninitialized_copy(__position, _M_finish, __new_finish); // 3
			}
			__STL_UNWIND((destroy(__new_start,__new_finish) , _M_deallocate(__new_start,__len)));
