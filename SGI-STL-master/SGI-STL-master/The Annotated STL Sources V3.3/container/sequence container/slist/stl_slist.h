...
// allocator instance if it isn't necessary.
	...
	protected:
		...
		typename _Alloc_traits<_Slist_node<_Tp>,_Allocator>::allocator_type _M_node_allocator;
	...
	protected:
		typedef typename _Alloc_traits<_Slist_node<_Tp>, _Allocator>::_Alloc_type _Alloc_type;
	...
	struct _Slist_base    : public _Slist_alloc_base<_Tp , _Alloc , _Alloc_traits<_Tp, _Alloc>::_S_instanceless >
	{
	...
	protected:
		_Slist_node_base _M_head; // 头部，不是指针，不是链表实际节点，哑节点(它的next，代表的是第一个节点)
		
	};  

#endif /* __STL_USE_STD_ALLOCATORS */

template <class _Tp, class _Alloc> _Slist_node_base* _Slist_base<_Tp,_Alloc>::_M_erase_after( _Slist_node_base* __before_first , _Slist_node_base* __last_node) 
{
...
public:
	...
	typedef _Slist_iterator<_Tp, _Tp&, _Tp*>             iterator; //迭代器
	typedef _Slist_iterator<_Tp, const _Tp&, const _Tp*> const_iterator;
...
public:
	...
	slist(_InputIterator __first, _InputIterator __last , const allocator_type& __a =  allocator_type() ) : _Base(__a)
	{ 
		_M_insert_after_range(&this->_M_head, __first, __last);
	}

#else /* __STL_MEMBER_TEMPLATES */
	slist(const_iterator __first, const_iterator __last , const allocator_type& __a =  allocator_type()) : _Base(__a)
	{
		_M_insert_after_range( &this->_M_head , __first , __last );
	}
	
	slist(const value_type* __first , const value_type* __last , const allocator_type& __a =  allocator_type() ) : _Base(__a)
	{
		_M_insert_after_range( &this->_M_head , __first , __last ); 
	}
...
public:
	...
	void pop_front()
	{
		...
		destroy(&__node->_M_data); //析构
		this->_M_put_node(__node); //释放空间
	}
...
private:
	...
	// 在 __pos 位置后插入 n 个 __x
	template <class _Integer>
	void _M_insert_after_range(_Node_base* __pos, _Integer __n, _Integer __x, __true_type)
	{
	...
	template <class _InIter>
	void _M_insert_after_range(_Node_base* __pos ,       _InIter __first , _InIter __last , __false_type) 
	{
...
slist<_Tp,_Alloc>& slist<_Tp,_Alloc>::operator=(const slist<_Tp,_Alloc>& __x)
{
	if (&__x != this)
	{
		...
		if (__n2 == 0)
			...
			_M_insert_after_range(__p1, const_iterator((_Node*)__n2),  const_iterator(0));
	}
	return *this;
}
...
template <class _Tp, class _Alloc>
inline bool operator==(const slist<_Tp,_Alloc>& _SL1, const slist<_Tp,_Alloc>& _SL2)
{
...
void slist<_Tp,_Alloc>::unique(_BinaryPredicate __pred)
{
	...
	if (__cur) 
	{
		while (__cur->_M_next) 
		{
			if ( __pred(((_Node*)__cur)->_M_data , ((_Node*)(__cur->_M_next))->_M_data) )
				this->_M_erase_after(__cur);
			else
