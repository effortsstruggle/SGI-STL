...
// allocator instance if it isn't necessary.
	...
	public:
		...
		typedef typename _Alloc_traits<_Tp, _Allocator>::allocator_type  allocator_type; //空间配置器
...
[]
	...
	public:
		...
		// 配置一个节点空间，令 _M_node 指向它，令 _M_node 头尾都指向自己，不设元素值 （空节点）
...
public:
	...
	iterator insert(iterator __position, const _Tp& __x) 
	{
		...
		__tmp->_M_next = __position._M_node;  // list为双向环形链表
	...
	iterator erase(iterator __position)
	{
		...
		_Destroy(&__n->_M_data); //析构节点数据

		_M_put_node(__n); //释放空间
	...
	{ 
		this->insert( this->begin() , __n , __value );
	}
	
	explicit list(size_type __n) : _Base( allocator_type() )
	{ 
		this->insert( this->begin() , __n , _Tp() ); 
	}
	...
	list(const _Tp* __first, const _Tp* __last , const allocator_type& __a = allocator_type()) 	: _Base(__a)
	{ 
		this->insert(begin(), __first, __last); 
	}
	list(const_iterator __first, const_iterator __last, const allocator_type& __a = allocator_type()) 	: _Base(__a)
	{
		this->insert(begin(), __first, __last); 
	}
...
protected:
	// 将 [first, last) 内的所有元素移动到 position 之前 （改变指针的指向）
...
{
  for ( ; __first != __last; ++__first)
    this->insert ( __position , *__first );
}

template <class _Tp, class _Alloc>
void list<_Tp, _Alloc>::insert(iterator __position , const_iterator __first , const_iterator __last)
{
  for ( ; __first != __last; ++__first)
    this->insert( __position ,  *__first );
}
...
void list<_Tp, _Alloc>::merge(list<_Tp, _Alloc>& __x)
{
  ...
  if (__first2 != __last2) 
  	transfer(__last1, __first2, __last2); // 将剩余的list2连接到list1上
}
